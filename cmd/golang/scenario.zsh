...
ci_compose_memory=${ci_compose_memory:-1024M}
...
# ls ./target
mvn_bash_c="mvn clean package -s ~/m2_settings.xml -q -Dmaven.test.skip=true -f pom.xml"
docker exec -i -w $PWD maven3-java8.dev bash -c "${mvn_bash_c}"

LAST_ERROR_CODE=$?
if [ ! "$LAST_ERROR_CODE" = "0" ]
then
  echo ">>❌ mvn clean package fail(@docker): $LAST_ERROR_CODE"
...
JAVA_OPS=`echo "$ci_compose_memory" | tr -cd "[0-9]"`
JAVA_OPS=-Xmx$[$JAVA_OPS/2]m
...
then
    ...
    docker service update -d --env-add "JAVA_OPS=${JAVA_OPS}" \
    ...
    docker service create -d --mode replicated -e "JAVA_OPS=${JAVA_OPS}" \
