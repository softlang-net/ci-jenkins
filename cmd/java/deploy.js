...
const
    ...
    ci_container_compiler = getEnv('ci_container_compiler', 'java-complier'),
    ci_java_ops = getEnv('ci_java_ops', '-Xms128m -Xmx128m'),
    ...
    // compiler config
    ci_dockerfile = getEnv('ci_dockerfile', '/opt/make/compilers/java/Dockerfile'),
...
// 📌 compiler: maven make target/app.jar
env = {
    DOCKER_CONTEXT: ci_docker_context_build,
    work_dir: `${ci_work_dir}/src/${ci_git_src_dir}`,
    cmd_compiler: `mvn dependency:get  -DgroupId=junit -DartifactId=junit -Dversion=1.0 -s ~/m2_settings.xml`
}
printLog(JSON.stringify(env, null, 2))
if (env3) {
    exec("📌 compiler: maven make target/app.jar", env,
...
env = {
    ...
    cmd_image_build: `docker build --force-rm --compress` +
        ...
        ` --build-arg ci_java_ops=${ci_java_ops}` +
